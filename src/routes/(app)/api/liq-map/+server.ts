...
export async function GET({ request, locals: { supabase, user } }: RequestEvent) {
	...
	if (!baseAsset) {
		return error(400, 'baseAsset parameter must be specified!');
	}

	if (!quoteAsset) {
		return error(400, 'quoteAsset parameter must be specified!');
	}

	console.log('liq map');
	console.log({ timeframe, exchange, baseAsset, quoteAsset });

	const liquidationData = await fetchCoinglassLiqMap(timeframe, exchange, baseAsset + quoteAsset);
